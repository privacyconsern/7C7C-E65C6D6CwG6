...
<div class="justify-center items-center flex flex-col mt-50 mb-50">
    <form method="POST" action="?/register" class="grid grid-cols-1 gap-4 content-center" use:enhance={handleSubmit}>
        <label>
        Email:
        <input
            type="email" 
    ...
    <label>
        ...
        <input type="password" 
            id="password" 
            name="password" bind:value={password} required />
    </label>
    <label>
        Confirm Password:
        <input type="password" 
            id="confirmPassword" 
            name="confirmPassword" bind:value={confirmPassword} required />
    </label>
    <Button />
</form>
</div>
