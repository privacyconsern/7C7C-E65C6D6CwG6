  ...
  if (project) {
    ...
    if (floor && floor.walls.length > 0) {
      ...
      for (let ri = 0; ri < rooms.length; ri++) {
        ...
        ctx.fillText(
          formatArea(room.area, get(projectSettings).units),
          c.x - minX + pad,
          c.y - minY + pad + 14,
        );
      }
...
export function exportAsSVG(project: Project) {
  ...
  if (floor.measurements) {
    for (const m of floor.measurements) {
      const x1 = m.x1 - minX + pad,
        y1 = m.y1 - minY + pad;
      const x2 = m.x2 - minX + pad,
        y2 = m.y2 - minY + pad;
  ...
  if (floor.annotations) {
    for (const a of floor.annotations) {
      const ax1 = a.x1 - minX + pad,
        ay1 = a.y1 - minY + pad;
      const ax2 = a.x2 - minX + pad,
        ay2 = a.y2 - minY + pad;
      const dx = ax2 - ax1,
        dy = ay2 - ay1;
      const len = Math.hypot(dx, dy);
      ...
      const d1x = ax1 + nx * offset,
        d1y = ay1 + ny * offset;
      const d2x = ax2 + nx * offset,
        d2y = ay2 + ny * offset;
      // Leader lines
  ...
  if (floor.textAnnotations) {
    for (const ta of floor.textAnnotations) {
      ...
      const transform = ta.rotation
        ? ` transform="rotate(${ta.rotation} ${tx} ${ty})"`
        : "";
...
export function exportPDF(project: Project) {
  ...
  if (rooms.length > 0) {
    ...
    pdf.text(
      formatArea(totalArea, settings.units),
      tX + colWidths[0] + colWidths[1] + colWidths[2] + 3,
      tY + 5.5,
    );
    pdf.setTextColor(0);
    ...
    pdf.setTextColor(80);
    pdf.text(
      `${rooms.length} rooms  ·  ${floor.walls.length} walls  ·  ${floor.doors.length} doors  ·  ${floor.windows.length} windows  ·  ${floor.furniture.length} furniture items`,
      tX,
      tY,
    );
  ...
  if (threeDCanvas && threeDCanvas.width > 10 && threeDCanvas.height > 10) {
    try {
      ...
      /* 3D canvas tainted or unavailable — skip */
    }
  }
