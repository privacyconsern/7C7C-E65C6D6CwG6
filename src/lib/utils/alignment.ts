  ...
  switch (op) {
    case "align-left": {
      ...
      for (const r of rects)
        updates.set(r.item.id, { x: minX + r.w / 2, y: r.y });
      break;
    }
    case "align-right": {
      const maxX = Math.max(...rects.map((r) => r.x + r.w / 2));
      for (const r of rects)
        updates.set(r.item.id, { x: maxX - r.w / 2, y: r.y });
      break;
    }
    case "align-top": {
      const minY = Math.min(...rects.map((r) => r.y - r.d / 2));
      for (const r of rects)
        updates.set(r.item.id, { x: r.x, y: minY + r.d / 2 });
      break;
    }
    case "align-bottom": {
      const maxY = Math.max(...rects.map((r) => r.y + r.d / 2));
      for (const r of rects)
        updates.set(r.item.id, { x: r.x, y: maxY - r.d / 2 });
      break;
    }
    case "align-center-h": {
      const minX = Math.min(...rects.map((r) => r.x - r.w / 2));
