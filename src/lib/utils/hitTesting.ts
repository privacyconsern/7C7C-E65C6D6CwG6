...
export function positionOnWall(p: Point, w: Wall): number {
  ...
  return Math.max(
    0.1,
    Math.min(0.9, ((p.x - w.start.x) * dx + (p.y - w.start.y) * dy) / len2),
  );
}
  ...
  for (const w of walls) {
    if (w.curvePoint) {
      for (let i = 0; i <= 20; i++) {
        ...
        if (Math.hypot(p.x - pt.x, p.y - pt.y) < threshold + w.thickness / 2)
          return w;
      }
    } else {
...
export function findHandleAt(
  ...
  if (Math.abs(rx) < ht && Math.abs(ry - (-hd - rotHandleDist)) < ht)
    return "rotate";
...
export function findColumnAt(
  ...
  for (const col of [...columns].reverse()) {
    ...
    if (col.shape === "round") {
      ...
      const rx = dx * Math.cos(angle) - dy * Math.sin(angle);
      const ry = dx * Math.sin(angle) + dy * Math.cos(angle);
      if (Math.abs(rx) < col.diameter / 2 && Math.abs(ry) < col.diameter / 2)
        return col;
    }
  }
  return null;
...
export function findStairAt(
  ...
  for (const stair of [...stairs].reverse()) {
    ...
    const rx = dx * Math.cos(angle) - dy * Math.sin(angle);
    const ry = dx * Math.sin(angle) + dy * Math.cos(angle);
    if (Math.abs(rx) < stair.width / 2 && Math.abs(ry) < stair.depth / 2)
      return stair;
  }
  return null;
}
...
export function hitTestTextAnnotation(
  ...
  for (let i = floor.textAnnotations.length - 1; i >= 0; i--) {
    ...
    const pad = 8 / zoom;
    if (Math.abs(dx) < maxW / 2 + pad && Math.abs(dy) < totalHeight / 2 + pad)
      return ta.id;
  }
  return null;
}
