...
function getOrCreate(
  id: string,
  size: number,
  draw: (cx: CanvasRenderingContext2D, s: number) => void,
): HTMLCanvasElement {
...
export function generateBrickTexture(
  baseColor: string = "#8B4513",
  variant: "standard" | "exposed" = "standard",
): HTMLCanvasElement {
  ...
  return getOrCreate(id, 1024, (cx, S) => {
    ...
    for (let row = 0; row < S / brickH + 1; row++) {
      ...
      for (let col = -1; col < S / brickW + 2; col++) {
        ...
        cx.fillRect(
          x + mortarW / 2,
          y + brickH - mortarW / 2 - 2,
          brickW - mortarW,
          2,
        );
        cx.fillRect(
          x + brickW - mortarW / 2 - 2,
          y + mortarW / 2,
          2,
          brickH - mortarW,
        );
    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        ...
        for (let p = 0; p < 8; p++) {
          ...
          cx.fillStyle =
            rng() > 0.5
              ? `rgba(0,0,0,${0.03 + rng() * 0.08})`
              : `rgba(255,255,255,${0.03 + rng() * 0.06})`;
          cx.beginPath();
          cx.ellipse(
            px,
            py,
...
export function generateWoodPanelTexture(
  baseColor: string = "#8B6914",
): HTMLCanvasElement {
...
export function generateSubwayTileTexture(
...
export function generateHardwoodTexture(
  baseColor: string = "#c4a882",
): HTMLCanvasElement {
      ...
      for (let y = -plankH; y < S + plankH; y += plankH) {
        ...
        for (let g = 0; g < 30; g++) {
          ...
          for (let gx = 0; gx < plankW - gap; gx += 8) {
            cx.lineTo(
              x + gap + gx,
              gy + Math.sin(gx * 0.02 + rng() * 5) * (1 + rng() * 2),
            );
          }
          cx.stroke();
        }
...
export function getFloorTextureCanvas(
  materialId: string,
): HTMLCanvasElement | null {
...
function notifyTextureLoad() {
  for (const cb of textureLoadCallbacks) cb();
}
export function setTextureLoadCallback(cb: () => void) {
  textureLoadCallbacks.add(cb);
}

export function getWallTextureCanvas(
  textureId: string,
  color: string,
): HTMLCanvasElement | null {
  ...
  switch (textureId) {
    case "red-brick":
      return generateBrickTexture(color, "standard");
    case "exposed-brick":
      return generateBrickTexture(color, "exposed");
    case "stone":
      return generateStoneTexture(color);
    case "wood-panel":
      return generateWoodPanelTexture(color);
    case "concrete-block":
      return generateConcreteTexture(color);
    case "subway-tile":
      return generateSubwayTileTexture(color);
    default:
      return null;
  }
}
