...
function splitWallsAtTJunctions(walls: Wall[]): Edge[] {
  ...
  for (let wi = 0; wi < walls.length; wi++) {
    ...
    for (const ep of endpoints) {
      ...
      if (
        t <= EPSILON / Math.sqrt(lenSq) ||
        t >= 1 - EPSILON / Math.sqrt(lenSq)
      )
        continue;
      ...
      if (dist < EPSILON) {
        ...
        const already = splitWalls[wi].splitPoints.some((sp) =>
          ptEq(sp.point, ep),
        );
        if (!already) {
...
export function detectRooms(walls: Wall[]): Room[] {
  ...
  const adj = new Map<
    number,
    { to: number; wallId: string; angle: number }[]
  >();
    ...
    for (const [from, to] of [
      ...
      for (let step = 0; step < 20; step++) {
        ...
        const inAngle = Math.atan2(
          vertices[cur].y - vertices[next].y,
          vertices[cur].x - vertices[next].x,
        );
        const neighbors2 = adj.get(next);
        if (!neighbors2 || neighbors2.length === 0) {
          valid = false;
          break;
        }
      ...
      if (!valid || cycle[cycle.length - 1] !== from || cycle.length < 4)
        continue;
      ...
      const dup = rooms.some((r) => {
        ...
        return (
          uniqueWalls.length === rw.size && uniqueWalls.every((w) => rw.has(w))
        );
      });
      if (dup) continue;
...
export function getRoomPolygon(room: Room, walls: Wall[]): Point[] {
  ...
  for (let i = 0; i < roomWalls.length - 1; i++) {
    verts.push(tip);
    const next = roomWalls.find(
      (w) => !used.has(w.id) && (ptEq(w.start, tip) || ptEq(w.end, tip)),
    );
    if (!next) break;
    used.add(next.id);
