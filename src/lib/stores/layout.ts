...
export const activeFloor = derived(currentProject, ($p) => {
  if (!$p) return null;
  return (
    $p.floors.find((f) => f.id === $p.activeFloorId) ?? $p.floors[0] ?? null
  );
});
...
export const undoHistoryStore = writable<{
  entries: { description: string; timestamp: number }[];
  currentIndex: number;
}>({ entries: [], currentIndex: -1 });

function syncHistoryStore() {
  const entries = undoStack.map((e) => ({
...
export function endUndoGroup(description?: string) {
  ...
  if (undoGroupDepth === 0 && undoGroupSnapshot !== null) {
    undoStack.push({
      state: undoGroupSnapshot,
      description: description || _nextDescription || "Group action",
      timestamp: Date.now(),
    });
...
function snapshot(description?: string) {
  ...
  if (p)
    undoStack.push({
      ...
      description: description || _nextDescription || "Edit",
      timestamp: Date.now(),
    });
  if (undoStack.length > 50) undoStack.shift();
  redoStack.length = 0;
...
function reviveDates(p: Project): Project {
  if (p.createdAt && !(p.createdAt instanceof Date))
    p.createdAt = new Date(p.createdAt as any);
  if (p.updatedAt && !(p.updatedAt instanceof Date))
    p.updatedAt = new Date(p.updatedAt as any);
  return p;
}
...
export function addWindow(
  wallId: string,
  position: number,
  windowType: import("$lib/models/types").Window["type"] = "standard",
): string {
  const id = uid();
  const defaults: Record<
    import("$lib/models/types").Window["type"],
...
export function importFloorIntoCurrentProject(
  ...
  if (floor.stairs)
    existing.stairs = [...(existing.stairs || []), ...floor.stairs];
  if (floor.columns)
    existing.columns = [...(existing.columns || []), ...floor.columns];
  currentProject.set({ ...p });
}
...
export const placingWindowType =
  writable<import("$lib/models/types").Window["type"]>("standard");
...
export function duplicateFurniture(id: string): string | null {
  ...
  mutate((f) => {
    f.furniture.push({
      ...fi,
      id: newId,
      position: { x: fi.position.x + 30, y: fi.position.y + 30 },
    });
  });
  return newId;
}
...
export function duplicateWall(id: string): string | null {
  ...
  mutate((f) => {
    f.walls.push({
      ...w,
      id: newId,
      start: { x: w.start.x + 30, y: w.start.y + 30 },
      end: { x: w.end.x + 30, y: w.end.y + 30 },
    });
  });
  return newId;
}
...
export function findGroupForElement(
  floor: Floor,
  elementId: string,
