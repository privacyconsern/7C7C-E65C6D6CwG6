<script lang="ts">
    ...
    import {
        ...
        manualSave,
        initAutoSave,
    } from "$lib/stores/saveStatus";
    import {
        initVersionHistory,
        snapshotOnAction,
    } from "$lib/stores/versionHistory";
    ...
    function updateLastSavedText() {
        ...
        if (diff < 5) lastSavedText = "Last saved: just now";
            ...
            lastSavedText = `Last saved: ${Math.floor(diff / 60)} min ago`;
    ...
    function onExport3DPNG() {
        ...
        setTimeout(() => {
            const c = document.querySelector(
                ".w-full.h-full canvas, div canvas",
    ...
    onMount(() => {
        ...
        function handleClickOutside(e: MouseEvent) {
            if (
                exportOpen &&
                exportRef &&
                !exportRef.contains(e.target as Node)
            ) {
                exportOpen = false;
            }
        }
        function handleKeydown(e: KeyboardEvent) {
            ...
            if (e.key === "Escape" && versionHistoryOpen)
...
<div
    class="h-12 bg-gradient-to-r from-slate-800 to-slate-700 flex items-center px-4 gap-3 shrink-0 shadow-sm"
>
    <!-- Back to Projects -->
    <a
        href="/"
    ...
    {:else}
        <button
            ...
            title="Click to rename">{projectName}</button
        >
    {/if}
    ...
    <div class="flex items-center gap-1">
        {#each floors as fl}
            <button
                class="px-2 py-0.5 text-xs rounded transition-colors {fl.id ===
                activeFloorId
                    ? 'bg-white text-slate-800 font-semibold'
                    : 'text-white/80 hover:bg-white/10'}"
                onclick={() => setActiveFloor(fl.id)}
                ondblclick={() => onRemoveFloor(fl.id)}
                title={fl.id === activeFloorId
                    ? "Active floor (dbl-click to remove)"
                    : "Click to switch, dbl-click to remove"}>{fl.name}</button
            >
        {/each}
        <button
            onclick={onAddFloor}
    ...
    <button
        ...
        class="p-1.5 rounded transition-colors {snapOn
    ...
    {#if mode === "2d"}
        <div class="flex bg-white/15 rounded-full p-0.5">
            <button
                ...
                class="px-2 py-1 text-xs font-semibold rounded-full transition-colors {!$panMode
            ...
            >
                <svg
                    ...
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    ><path d="M18 11V6a2 2 0 0 0-4 0v1" /><path
                        d="M14 10V4a2 2 0 0 0-4 0v2"
    ...
    <button
        onclick={() =>
            layerVisibility.update((v) => ({ ...v, furniture: !v.furniture }))}
        class="p-1.5 rounded transition-colors {$showFurnitureStore
            ? 'text-white bg-white/20'
            : 'text-white/40 hover:text-white/70 hover:bg-white/10'}"
    ...
    >
        <svg
            ...
            stroke-linejoin="round"
        >
            <rect x="2" y="12" width="20" height="8" rx="1" /><path
    ...
    <div class="flex bg-white/15 rounded-full p-0.5">
        <button
            onclick={() => setMode("2d")}
            class="px-3 py-1 text-xs font-semibold rounded-full transition-colors {mode ===
            '2d'
                ? 'bg-white text-slate-800'
                : 'text-white/80 hover:text-white'}">2D</button
        >
        <button
            onclick={() => setMode("3d")}
            class="px-3 py-1 text-xs font-semibold rounded-full transition-colors {mode ===
            '3d'
                ? 'bg-white text-slate-800'
                : 'text-white/80 hover:text-white'}">3D</button
        >
    </div>
    ...
    {#if mode === "2d"}
        <div class="flex items-center gap-1 bg-white/15 rounded-full p-0.5">
            <button
                onclick={() =>
                    canvasZoom.update((z) => Math.max(0.1, z / 1.25))}
    ...
    <div class="relative" bind:this={exportRef}>
        <button
            onclick={() => {
                ...
                if (exportOpen) triggerTip("first-export", 300, 60);
            }}
        ...
        {#if exportOpen}
            <div
                class="absolute right-0 top-full mt-1 bg-white rounded-lg shadow-lg border border-gray-200 py-1 w-48 z-50"
            >
                <button
                    class="w-full px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 text-left flex items-center gap-2"
                    onclick={() => {
                        exportOpen = false;
                ...
                >
                    <svg
                        ...
                        /><path d="M14 2v6h6" /><path d="M8 16h2" /><path
                            d="M14 16h2"
                        /></svg
                    >
                    Export as DXF
                ...
                >
                    <svg
                        ...
                        ><path
                            d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"
                        /><path d="M14 2v6h6" /><path d="M9 16h6" /></svg
                    >
                    Export as DWG
                </button>
                <button
                ...
                >
                    <svg
                        ...
                        ><path
                            d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"
                        /><path d="M14 2v6h6" /><path d="M16 11v6" /><path
                            d="M8 11v6"
                        /><path d="M12 11v6" /></svg
                    >
                    Export as PDF
    ...
    <span
        class="text-[11px] font-medium transition-all duration-300 {$saveState ===
        'saved'
            ? 'text-emerald-400'
            : $saveState === 'saving'
    ...
    <button
        onclick={save}
        class="px-3 py-1.5 text-sm bg-white text-slate-800 font-semibold rounded-lg hover:bg-blue-50 transition-colors shadow-sm"
    >
        Save
    </button>
</div>
...
{#if areaOpen}
    ...
    <div
        ...
        onclick={() => (areaOpen = false)}
        onkeydown={(e) => {
            if (e.key === "Escape") areaOpen = false;
        }}
    >
        <!-- svelte-ignore a11y_no_static_element_interactions -->
        <div
            class="bg-white rounded-xl shadow-2xl w-[420px] max-h-[80vh] overflow-hidden"
