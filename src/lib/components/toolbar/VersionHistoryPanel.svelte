<script lang="ts">
    ...
    import { currentProject } from "$lib/stores/layout";
    import { get } from "svelte/store";

    let { open = $bindable(false) } = $props();

    let snapshots: Snapshot[] = $state([]);
    snapshotsStore.subscribe((v) => {
        snapshots = v;
    });
...
{#if open}
    ...
    >
        <!-- svelte-ignore a11y_no_static_element_interactions -->
        <div
            class="bg-white rounded-xl shadow-2xl w-96 max-h-[70vh] flex flex-col"
            ...
            <div class="flex-1 overflow-y-auto px-4 py-2">
                {#if snapshots.length === 0}
                    <p class="text-sm text-gray-400 text-center py-8">
                        No snapshots yet.<br />Versions are saved automatically
                        every 5 minutes.
                    </p>
                {:else}
                    <div class="space-y-1">
                        {#each [...snapshots].reverse() as snap, i}
                            ...
                            <div
                                class="flex items-center justify-between py-2 px-2 rounded-lg hover:bg-gray-50 group transition-colors"
                            >
