<script lang="ts">
    ...
    activeFloor.subscribe((f) => {
        floor = f;
    });
    selectedElementId.subscribe((id) => {
        selId = id;
    });
    selectedRoomId.subscribe((id) => {
        selRoomId = id;
    });
    detectedRoomsStore.subscribe((rooms) => {
        detectedRooms = rooms;
    });
    ...
    let selectedWall = $derived(
        floor?.walls?.find((w) => w.id === selId) ?? null,
    );
    let selectedDoor = $derived(
        floor?.doors?.find((d) => d.id === selId) ?? null,
    );
    let selectedWindow = $derived(
        floor?.windows?.find((w) => w.id === selId) ?? null,
    );
    let selectedFurniture = $derived(
        floor?.furniture?.find((f) => f.id === selId) ?? null,
    );
    let selectedStair = $derived(
        floor?.stairs?.find((s) => s.id === selId) ?? null,
    );
    let selectedColumn = $derived(
        floor?.columns?.find((c) => c.id === selId) ?? null,
    );
    let selectedTextAnnotation = $derived(
        floor?.textAnnotations?.find((t) => t.id === selId) ?? null,
    );
    let hasBgImage = $derived(!!floor?.backgroundImage);
    let selectedRoom = $derived(
        floor?.rooms?.find((r) => r.id === selRoomId) ??
            detectedRooms.find((r) => r.id === selRoomId) ??
            null,
    );

    // Helper to get the parent wall for selected door/window
    let selectedDoorWall = $derived(
        (selectedDoor &&
            floor?.walls?.find((w) => w.id === selectedDoor.wallId)) ??
            null,
    );
    let selectedWindowWall = $derived(
        (selectedWindow &&
            floor?.walls?.find((w) => w.id === selectedWindow.wallId)) ??
            null,
    );
    ...
    function calcWallLength(wall: Wall): number {
        if (wall.curvePoint) {
            ...
            for (let i = 1; i <= N; i++) {
                ...
                const nx =
                    mt * mt * wall.start.x +
                    2 * mt * t * wall.curvePoint.x +
                    t * t * wall.end.x;
                const ny =
                    mt * mt * wall.start.y +
                    2 * mt * t * wall.curvePoint.y +
                    t * t * wall.end.y;
    ...
    let wallLength = $derived(
        selectedWall ? Math.round(calcWallLength(selectedWall)) : 0,
    );

    // Calculate door distances
    let doorDistFromA = $derived(
        selectedDoor && selectedDoorWall
            ? Math.round(
                  calcWallLength(selectedDoorWall) * selectedDoor.position,
              )
            : 0,
    );
    let doorDistFromB = $derived(
        selectedDoor && selectedDoorWall
            ? Math.round(
                  calcWallLength(selectedDoorWall) *
                      (1 - selectedDoor.position),
              )
            : 0,
    );

    // Calculate window distances
    let windowDistFromA = $derived(
        selectedWindow && selectedWindowWall
            ? Math.round(
                  calcWallLength(selectedWindowWall) * selectedWindow.position,
              )
            : 0,
    );
    let windowDistFromB = $derived(
        selectedWindow && selectedWindowWall
            ? Math.round(
                  calcWallLength(selectedWindowWall) *
                      (1 - selectedWindow.position),
              )
            : 0,
    );
    ...
    function onWallThickness(e: Event) {
        ...
        updateWall(selectedWall.id, {
            thickness: inputToCm(Number((e.target as HTMLInputElement).value)),
        });
    }
    function onWallHeight(e: Event) {
        if (!selectedWall) return;
        updateWall(selectedWall.id, {
            height: inputToCm(Number((e.target as HTMLInputElement).value)),
        });
    }
    ...
    function onDoorWidth(e: Event) {
        ...
        updateDoor(selectedDoor.id, {
            width: Math.max(
                1,
                inputToCm(Number((e.target as HTMLInputElement).value)) || 1,
            ),
        });
    }
    ...
    function onDoorType(e: Event) {
        if (!selectedDoor) return;
        updateDoor(selectedDoor.id, {
            type: (e.target as HTMLSelectElement).value as Door["type"],
        });
    }
    function onDoorSwing(e: Event) {
        if (!selectedDoor) return;
        updateDoor(selectedDoor.id, {
            swingDirection: (e.target as HTMLSelectElement).value as
                | "left"
                | "right",
        });
    }
    function flipDoorHorizontal() {
        ...
        updateDoor(selectedDoor.id, {
            swingDirection:
                selectedDoor.swingDirection === "left" ? "right" : "left",
        });
    }
    function flipDoorVertical() {
        if (!selectedDoor) return;
        updateDoor(selectedDoor.id, {
            flipSide: !(selectedDoor.flipSide ?? false),
        });
    }
    function onWindowType(e: Event) {
        if (!selectedWindow) return;
        updateWindow(selectedWindow.id, {
            type: (e.target as HTMLSelectElement).value as Win["type"],
        });
    }
    ...
    function onWindowSill(e: Event) {
        if (!selectedWindow) return;
        updateWindow(selectedWindow.id, {
            sillHeight: inputToCm(Number((e.target as HTMLInputElement).value)),
        });
    }
    ...
    function onDoorDistFromA(e: Event) {
        ...
        const newDistFromA = inputToCm(
            Number((e.target as HTMLInputElement).value),
        );
        const wallLen = calcWallLength(selectedDoorWall);
        const newPosition = Math.max(
            0.05,
            Math.min(0.95, newDistFromA / wallLen),
        );
    ...
    function onDoorDistFromB(e: Event) {
        if (!selectedDoor || !selectedDoorWall) return;
        const newDistFromB = inputToCm(
            Number((e.target as HTMLInputElement).value),
        );
        const wallLen = calcWallLength(selectedDoorWall);
        const newPosition = Math.max(
            0.05,
            Math.min(0.95, 1 - newDistFromB / wallLen),
        );
        updateDoor(selectedDoor.id, { position: newPosition });
    }
    ...
    function updateDetectedRoom(
        id: string,
        updates: Partial<{ name: string; floorTexture: string; color: string }>,
    ) {
    ...
    const textureGroups = [
        {
            label: "🪵 Wood",
            ids: ["light-oak", "walnut", "bamboo", "laminate"],
        },
        {
            label: "🔲 Tile",
            ids: ["ceramic-white", "ceramic-gray", "porcelain", "vinyl"],
        },
        {
            label: "🪨 Stone",
            ids: ["marble-white", "marble-dark", "concrete", "slate"],
        },
...
<div
    class="{is3D
        ? 'w-80'
        : 'w-64'} shrink-0 bg-white border-l border-gray-200 flex flex-col overflow-y-auto p-3 fixed right-0 z-40 shadow-lg"
    class:hidden={!hasSelection}
    style="top: 48px; bottom: 36px;"
>
    {#if selectedWall}
        <h3
        ...
        >
            <span
                class="w-6 h-6 bg-gray-200 rounded flex items-center justify-center text-xs"
                >▭</span
            >
            Wall Properties
        </h3>
        <div class="space-y-3">
            <label class="block">
                ...
                <input
                    type="number"
                    value={displayValue(wallLength)}
                    disabled
                    class="w-full px-2 py-1 border border-gray-200 rounded text-sm bg-gray-50"
                />
            </label>
            <label class="block">
                <span class="text-xs text-gray-500"
                    >Thickness ({unitLabel()})</span
                >
                <input
                    type="number"
                    value={displayValue(selectedWall.thickness)}
                    oninput={onWallThickness}
            ...
            <label class="block">
                ...
                <input
                    type="number"
                    value={displayValue(selectedWall.height)}
                    oninput={onWallHeight}
                    class="w-full px-2 py-1 border border-gray-200 rounded text-sm"
                />
            </label>
            <div class="flex items-center gap-2">
                ...
                <button
                    class="px-2 py-0.5 text-xs rounded {selectedWall.curvePoint
            ...
            <div>
                <div class="flex border-b border-gray-200 mb-3">
                    <button
                        class="flex-1 py-1.5 text-xs font-medium border-b-2 transition-colors {wallSideTab ===
                        'interior'
                ...
                {#if wallSideTab === "interior"}
                    {@const sideColor =
                        selectedWall.interiorColor || selectedWall.color}
                    {@const sideTex =
                        selectedWall.interiorTexture === "none"
                            ? undefined
                            : selectedWall.interiorTexture ||
                              selectedWall.texture}
                    <div class="space-y-2">
                        <span class="text-xs text-gray-500">Color</span>
                        <div class="grid grid-cols-6 gap-1.5">
                            {#each wallColors as wc}
                                <button
                                    class="w-7 h-7 rounded-md border-2 hover:border-gray-300 transition-colors {sideColor ===
                                    wc.color
                        ...
                        <div class="grid grid-cols-3 gap-1.5">
                            <button
                                class="p-1.5 rounded-md border-2 text-[10px] text-center h-14 {!sideTex
                                    ? 'border-blue-500 ring-1 ring-blue-200'
                                    : 'border-gray-200 hover:border-gray-300'}"
                                onclick={() => {
                                    if (selectedWall)
                                        updateWall(selectedWall.id, {
                                            interiorTexture: "none",
                                        });
                                }}>None</button
                            >
                                ...
                                {@const texPath =
                                    {
                                        ...
                                        "exposed-brick":
                                            "/textures/exposed-brick.jpg",
                                        stone: "/textures/stone.jpg",
                                        "wood-panel":
                                            "/textures/wood-panel.jpg",
                                        "concrete-block":
                                            "/textures/concrete.jpg",
                                        "subway-tile":
                                            "/textures/subway-tile.jpg",
                                    }[wc.id] ?? ""}
                                <button
                                    class="rounded-md border-2 text-[10px] text-center h-14 flex flex-col items-center justify-end overflow-hidden relative {sideTex ===
                                    wc.id
                                        ? 'border-blue-500 ring-1 ring-blue-200'
                                        : 'border-gray-200 hover:border-gray-300'}"
                                    style={texPath
                                        ? `background-image: url(${texPath}); background-size: cover; background-position: center;`
                                        : `background-color: ${wc.color}20`}
                                    onclick={() => {
                                        if (selectedWall)
                                            updateWall(selectedWall.id, {
                                                interiorTexture: wc.id,
                                                interiorColor: wc.color,
                                            });
                                    }}
                                    ><span
                                        class="bg-white/80 backdrop-blur-sm rounded px-1 py-0.5 mb-0.5 text-gray-700"
                                        >{wc.name}</span
                                    ></button
                                >
                            {/each}
                ...
                {:else}
                    {@const sideColor =
                        selectedWall.exteriorColor || selectedWall.color}
                    {@const sideTex =
                        selectedWall.exteriorTexture === "none"
                            ? undefined
                            : selectedWall.exteriorTexture ||
                              selectedWall.texture}
                    <div class="space-y-2">
                        <span class="text-xs text-gray-500">Color</span>
                        <div class="grid grid-cols-6 gap-1.5">
    ...
    {:else if selectedDoor}
        ...
        >
            <span
                class="w-6 h-6 bg-amber-100 rounded flex items-center justify-center text-xs"
                >🚪</span
            >
            Door Properties
        </h3>
        <div class="space-y-3">
            <label class="block">
                ...
                <input
                    type="number"
                    value={displayValue(selectedDoor.width)}
                    oninput={onDoorWidth}
                    min="1"
                    class="w-full px-2 py-1 border border-gray-200 rounded text-sm"
                />
            ...
            <label class="block">
                <span class="text-xs text-gray-500"
                    >Distance from A ({unitLabel()})</span
                >
                <input
                    type="number"
                    value={displayValue(doorDistFromA)}
                    oninput={onDoorDistFromA}
                    class="w-full px-2 py-1 border border-gray-200 rounded text-sm"
                />
            </label>
            <label class="block">
                <span class="text-xs text-gray-500"
                    >Distance from B ({unitLabel()})</span
                >
                <input
                    type="number"
                    value={displayValue(doorDistFromB)}
                    oninput={onDoorDistFromB}
                    class="w-full px-2 py-1 border border-gray-200 rounded text-sm"
                />
            </label>
            <label class="block">
                ...
                <input
                    type="number"
                    value={displayValue(selectedDoor.height ?? 210)}
                    oninput={onDoorHeight}
                    class="w-full px-2 py-1 border border-gray-200 rounded text-sm"
                />
            </label>
            <label class="block">
            ...
            <label class="block">
                ...
                <div class="flex gap-2">
                    <button
                        ...
                        class="flex-1 px-2 py-1.5 border rounded text-sm transition-colors {selectedDoor?.swingDirection ===
                        'left'
                            ? 'bg-blue-100 border-blue-400 text-blue-700'
                            : 'border-gray-200 hover:bg-gray-50'}">Left</button
                    >
                    <button
                        onclick={() => {
                            if (selectedDoor)
                        ...
                        'right'
                            ? 'bg-blue-100 border-blue-400 text-blue-700'
                            : 'border-gray-200 hover:bg-gray-50'}">Right</button
                    >
                </div>
            </label>
            <label class="block">
                ...
                <div class="flex gap-2">
                    <button
                        ...
                        class="flex-1 px-2 py-1.5 border rounded text-sm transition-colors {!selectedDoor?.flipSide
                            ? 'bg-blue-100 border-blue-400 text-blue-700'
                    ...
                    <button
                        onclick={() => {
                            if (selectedDoor)
                                updateDoor(selectedDoor.id, { flipSide: true });
                        }}
                        class="flex-1 px-2 py-1.5 border rounded text-sm transition-colors {selectedDoor?.flipSide
                            ? 'bg-blue-100 border-blue-400 text-blue-700'
                            : 'border-gray-200 hover:bg-gray-50'}"
    ...
    {:else if selectedWindow}
        ...
        >
            <span
                class="w-6 h-6 bg-cyan-100 rounded flex items-center justify-center text-xs"
                >🪟</span
            >
            Window Properties
        </h3>
        <div class="space-y-3">
            <label class="block">
                <span class="text-xs text-gray-500">Type</span>
                <select
                    value={selectedWindow.type ?? "standard"}
                    onchange={onWindowType}
                    class="w-full px-2 py-1 border border-gray-200 rounded text-sm"
                >
            ...
            <label class="block">
                ...
                <input
                    type="number"
                    value={displayValue(selectedWindow.width)}
                    oninput={onWindowWidth}
                    min="1"
                    class="w-full px-2 py-1 border border-gray-200 rounded text-sm"
                />
            </label>
            <label class="block">
                ...
                <input
                    type="number"
                    value={displayValue(windowDistFromA)}
                    oninput={onWindowDistFromA}
                    class="w-full px-2 py-1 border border-gray-200 rounded text-sm"
                />
            </label>
            <label class="block">
                ...
                <input
                    type="number"
                    value={displayValue(windowDistFromB)}
                    oninput={onWindowDistFromB}
                    class="w-full px-2 py-1 border border-gray-200 rounded text-sm"
                />
            </label>
            <label class="block">
                ...
                <input
                    type="number"
                    value={displayValue(selectedWindow.height)}
                    oninput={onWindowHeight}
                    class="w-full px-2 py-1 border border-gray-200 rounded text-sm"
                />
            </label>
            <label class="block">
                <span class="text-xs text-gray-500"
                    >Sill Height ({unitLabel()})</span
                >
                <input
                    type="number"
                    value={displayValue(selectedWindow.sillHeight)}
                    oninput={onWindowSill}
                    class="w-full px-2 py-1 border border-gray-200 rounded text-sm"
                />
            </label>
        </div>
    {:else if selectedFurniture}
        ...
        >
            <span
                class="w-6 h-6 bg-purple-100 rounded flex items-center justify-center text-xs"
            >
            ...
            <button
                onclick={() => {
                    if (selectedFurniture)
                        toggleFurnitureLock(selectedFurniture.id);
                }}
                class="ml-auto px-1.5 py-0.5 rounded text-xs border transition-colors {selectedFurniture.locked
                    ? 'bg-amber-100 border-amber-400 text-amber-700'
                    : 'border-gray-200 hover:bg-gray-50 text-gray-500'}"
                title={selectedFurniture.locked
                    ? "Unlock (Ctrl+L)"
                    : "Lock (Ctrl+L)"}
        ...
        <div class="space-y-3">
            ...
            <div>
                ...
                <div class="grid grid-cols-5 gap-1.5 mb-2">
                    {#each ["#ffffff", "#f5f5dc", "#d2b48c", "#daa520", "#8b4513", "#696969", "#191970", "#000000", "#dc143c", "#228b22"] as color}
                        <button
                            class="w-6 h-6 rounded border-2 hover:border-gray-300 transition-colors {(selectedFurniture.color ??
                                getCatalogItem(selectedFurniture.catalogId)
                                    ?.color) === color
                                ? 'border-blue-500 ring-1 ring-blue-200'
                                : 'border-gray-200'}"
            ...
            <div class="flex gap-1">
                ...
                <button
                    ...
                    class="flex-1 px-2 py-1.5 border border-gray-200 rounded text-sm hover:bg-gray-50 transition-colors"
                    title="Rotate 90° right">↻ 90°</button
                >
            </div>
            <div class="flex gap-1">
                <button
                    onclick={() => {
                        if (selectedFurniture) {
                            const s = selectedFurniture.scale;
                            updateFurniture(selectedFurniture.id, {
                                scale: { x: s.x * -1, y: s.y, z: s.z },
                            });
                        }
                    }}
                    class="flex-1 px-2 py-1.5 border border-gray-200 rounded text-sm hover:bg-gray-50 transition-colors"
                    title="Flip horizontally">↔ Flip H</button
                >
                <button
                    onclick={() => {
                        if (selectedFurniture) {
                            const s = selectedFurniture.scale;
                            updateFurniture(selectedFurniture.id, {
                                scale: { x: s.x, y: s.y * -1, z: s.z },
                            });
                        }
                    }}
                    class="flex-1 px-2 py-1.5 border border-gray-200 rounded text-sm hover:bg-gray-50 transition-colors"
                    title="Flip vertically">↕ Flip V</button
                >
            </div>
    ...
    {:else if selectedRoom}
        ...
        >
            <span
                class="w-6 h-6 bg-green-100 rounded flex items-center justify-center text-xs"
                >⬜</span
            >
            Room Properties
        </h3>
        <div class="space-y-3">
            ...
            <label class="block">
                ...
                <select
                    value={selectedRoom.roomType ?? "indoor"}
                    onchange={(e) => {
                        if (selectedRoom) {
                            const v = (e.target as HTMLSelectElement)
                                .value as RoomCategory;
                            updateRoom(selectedRoom.id, { roomType: v });
                            updateDetectedRoom(selectedRoom.id, {
                                roomType: v,
                            } as any);
                        }
                    }}
            ...
            <div>
                ...
                <p class="text-sm text-gray-700">
                    {formatArea(selectedRoom.area, settings.units)}
                </p>
            </div>
            <!-- Room Color -->
            <div>
                <span class="text-xs text-gray-500 mb-1.5 block"
                    >Room Color</span
                >
                <div class="grid grid-cols-5 gap-1.5 mb-2">
                    {#each roomColorPresets as preset}
                        <button
                            class="w-7 h-7 rounded-md border-2 hover:border-gray-300 transition-colors {selectedRoom.color ===
                            preset.color
                                ? 'border-blue-500 ring-1 ring-blue-200'
                                : 'border-gray-200'}"
                ...
                <div class="flex items-center gap-2">
                    ...
                    <input
                        ...
                        oninput={(e) =>
                            onRoomColor((e.target as HTMLInputElement).value)}
                        class="w-8 h-6 rounded border border-gray-200 cursor-pointer"
                    />
                </div>
            </div>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-3 gap-1.5">
                                {#each group.ids as matId}
                                    ...
                                    {#if mat}
                                        {@const texPath =
                                            floorTexPaths[mat.id] ?? ""}
                                        <button
                                            class="p-1 rounded-lg border-2 hover:border-gray-300 transition-all text-xs {selectedRoom.floorTexture ===
                                            mat.id
                                        ...
                                        >
                                            ...
                                            ></div>
                                            <div
                                                class="text-center leading-3 text-[10px] text-gray-600 truncate"
                                            >
                                                {mat.name}
                                            </div>
                                        </button>
    ...
    {:else if selectedStair}
        ...
        <div class="space-y-3">
            <label class="block">
                <span class="text-xs text-gray-500">Type</span>
                <select
                    value={selectedStair.stairType || "straight"}
                    onchange={(e) =>
                        updateStair(selectedStair!.id, {
                            stairType: (e.target as HTMLSelectElement)
                                .value as any,
                        })}
                    class="w-full px-2 py-1 border border-gray-200 rounded text-sm"
                >
            ...
            <label class="block">
                ...
                <input
                    type="number"
                    value={displayValue(selectedStair.width)}
                    oninput={(e) =>
                        updateStair(selectedStair!.id, {
                            width: inputToCm(
                                Number((e.target as HTMLInputElement).value),
            ...
            <label class="block">
                ...
                <input
                    type="number"
                    value={displayValue(selectedStair.depth)}
                    oninput={(e) =>
                        updateStair(selectedStair!.id, {
                            depth: inputToCm(
                                Number((e.target as HTMLInputElement).value),
            ...
            <label class="block">
                ...
                <input
                    type="number"
                    value={selectedStair.riserCount}
                    min="3"
                    max="30"
                    oninput={(e) =>
                        updateStair(selectedStair!.id, {
            ...
            <label class="block">
                ...
                <div class="flex gap-2">
                    <button
                        onclick={() =>
                            updateStair(selectedStair!.id, { direction: "up" })}
                        class="flex-1 px-2 py-1.5 border rounded text-sm transition-colors {selectedStair.direction ===
                        'up'
                            ? 'bg-blue-100 border-blue-400 text-blue-700'
                            : 'border-gray-200 hover:bg-gray-50'}">Up ↑</button
                    >
                    <button
                        onclick={() =>
                            updateStair(selectedStair!.id, {
    ...
    {:else if selectedColumn}
        ...
        <div class="space-y-3">
            <label class="block">
                ...
                <div class="flex gap-2">
                    <button
                        ...
                        class="flex-1 px-2 py-1.5 border rounded text-sm transition-colors {selectedColumn.shape ===
                        'round'
                            ? 'bg-blue-100 border-blue-400 text-blue-700'
                            : 'border-gray-200 hover:bg-gray-50'}"
                        >⭕ Round</button
            ...
            <label class="block">
                <span class="text-xs text-gray-500"
                    >{selectedColumn.shape === "round"
                        ? "Diameter"
                        : "Side Length"} ({unitLabel()})</span
                >
                <input
                    type="number"
                    value={displayValue(selectedColumn.diameter)}
                    min="10"
                    max="200"
                    oninput={(e) =>
                        updateColumn(selectedColumn!.id, {
            ...
            <label class="block">
                ...
                <input
                    type="number"
                    value={displayValue(selectedColumn.height)}
                    min="50"
                    max="1000"
                    oninput={(e) =>
                        updateColumn(selectedColumn!.id, {
            ...
            <div>
                ...
                <div class="grid grid-cols-5 gap-1.5 mb-2">
                    {#each columnColorPresets as preset}
                        <button
                            class="w-7 h-7 rounded-md border-2 hover:border-gray-300 transition-colors {selectedColumn.color ===
                            preset.color
                                ? 'border-blue-500 ring-1 ring-blue-200'
                                : 'border-gray-200'}"
                            style="background-color: {preset.color}"
    ...
    {:else if selectedTextAnnotation}
        <div class="space-y-3">
            ...
            >
                <span
                    class="w-6 h-6 bg-emerald-100 rounded flex items-center justify-center text-xs"
                    >🏷️</span
                >
                Text Annotation
            </h3>
            ...
            <label class="block">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-xs text-gray-400"
                        >{selectedTextAnnotation.color}</span
                    >
                </div>
            </label>
            <label class="block">
    ...
    {#if hasBgImage && floor?.backgroundImage}
        <div class="mt-4 pt-3 border-t border-gray-200">
            ...
            >
                <span
                    class="w-6 h-6 bg-blue-100 rounded flex items-center justify-center text-xs"
                    >🖼️</span
                >
                Background Image
            </h3>
            <div class="space-y-3">
                <label class="block">
                    ...
                    <input
                        ...
                        max="1"
                        step="0.05"
                        value={floor.backgroundImage.opacity}
                        oninput={(e) =>
                            updateBackgroundImage({
                                opacity: Number(
                                    (e.target as HTMLInputElement).value,
                ...
                <label class="block">
                    ...
                    <input
                        ...
                        max="5"
                        step="0.05"
                        value={floor.backgroundImage.scale}
                        oninput={(e) =>
                            updateBackgroundImage({
                                scale: Number(
                                    (e.target as HTMLInputElement).value,
                ...
                <label class="block">
                    ...
                    <input
                        type="number"
                        value={floor.backgroundImage.rotation}
                        oninput={(e) =>
                            updateBackgroundImage({
                                rotation: Number(
                                    (e.target as HTMLInputElement).value,
                ...
                <div class="flex gap-2">
                    <button
                        onclick={() =>
                            updateBackgroundImage({
                                locked: !floor!.backgroundImage!.locked,
                            })}
                        class="flex-1 px-2 py-1.5 border rounded text-sm {floor
                            .backgroundImage.locked
                            ? 'bg-amber-100 border-amber-400 text-amber-700'
                            : 'border-gray-200 hover:bg-gray-50'}"
                        >{floor.backgroundImage.locked
                            ? "🔒 Locked"
                            : "🔓 Unlocked"}</button
                    >
                    <button
