<script lang="ts">
    ...
    let categories: Category[] = $derived.by(() => {
        ...
        cats.push({
            ...
            label: "Doors",
            icon: "🚪",
            items: floor.doors.map((d, i) => ({
                id: d.id,
                label: `${d.type} door ${i + 1}`,
                icon: "🚪",
            })),
        });
        ...
        cats.push({
            ...
            label: "Windows",
            icon: "🪟",
            items: floor.windows.map((w, i) => ({
                id: w.id,
                label: `${w.type} window ${i + 1}`,
                icon: "🪟",
            })),
        });
        ...
        if (floor.stairs?.length) {
            cats.push({
                ...
                label: "Stairs",
                icon: "🪜",
                items: floor.stairs.map((s, i) => ({
                    id: s.id,
                    label: `Stair ${i + 1} (${s.direction})`,
                    icon: "🪜",
                })),
            });
        }
        ...
        if (floor.columns?.length) {
            cats.push({
                ...
                label: "Columns",
                icon: "🏛️",
                items: floor.columns.map((c, i) => ({
                    id: c.id,
                    label: `${c.shape} column ${i + 1}`,
                    icon: "🏛️",
                })),
            });
        }
        ...
        if (floor.guides?.length) {
            cats.push({
                ...
                label: "Guides",
                icon: "📏",
                items: floor.guides.map((g, i) => ({
                    id: g.id,
                    label: `${g.orientation} guide ${i + 1}`,
                    icon: g.orientation === "horizontal" ? "─" : "│",
                })),
            });
        }
            ...
            cats.push({
                ...
                items: floor.measurements.map((m, i) => {
                    const dist = Math.round(
                        Math.hypot(m.x2 - m.x1, m.y2 - m.y1),
                    );
                    return {
                        id: m.id,
                        label: `Measurement ${i + 1} (${dist} cm)`,
                        icon: "📐",
                    };
                }),
            });
...
<div
    class="w-56 bg-white border-l border-gray-200 flex flex-col overflow-hidden text-xs select-none"
>
    <div
        class="px-3 py-2 border-b border-gray-100 font-semibold text-gray-700 text-sm flex items-center gap-1.5"
    >
        🗂 Layers
    </div>
    <div class="flex-1 overflow-y-auto">
        {#each categories as cat}
            <div class="border-b border-gray-50 relative">
                ...
                >
                    <span class="text-[10px] text-gray-400 w-3"
                        >{collapsed[cat.key] ? "▸" : "▾"}</span
                    >
                    <span>{cat.icon}</span>
                ...
                {#if !collapsed[cat.key]}
                    ...
                    {#if cat.items.length === 0}
                        <div class="pl-7 pr-2 py-1 text-gray-300 italic">
                            Empty
                        </div>
                    {/if}
                {/if}
