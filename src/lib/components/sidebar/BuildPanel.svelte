<script lang="ts">
    ...
    import {
        furnitureCatalog,
        furnitureCategories,
    } from "$lib/utils/furnitureCatalog";
    ...
    selectedTool.subscribe((t) => {
        currentTool = t;
    });

    let currentPlacing = $state<string | null>(null);
    placingFurnitureId.subscribe((id) => {
        currentPlacing = id;
    });
    ...
    function onPresetClick(presetId: string, templateName?: string) {
        ...
        if (preset) {
            let cx = 0,
                cy = 0;
            canvasCamX.subscribe((v) => {
                cx = v;
            })();
            canvasCamY.subscribe((v) => {
                cy = v;
            })();
            const template = templateName
                ? (roomTemplates.find((t) => t.name === templateName) ?? null)
                : null;
    ...
    function addToRecent(id: string) {
        recentIds = [id, ...recentIds.filter((r) => r !== id)].slice(
            0,
            MAX_RECENT,
        );
    ...
    let recentItems = $derived(
        recentIds
            ...
            .filter(Boolean) as FurnitureDef[],
    );
    ...
    const doorCatalog: {
        ...
        },
        {
            ...
            name: "Bifold",
            desc: "180cm fold",
            icon: "M3 3h5v18H3zM9 3h6v18H9zM16 3h5v18h-5z",
        },
    ];
    ...
    placingStair.subscribe((v) => {
        isPlacingStair = v;
    });

    let isPlacingColumn = $state(false);
    placingColumn.subscribe((v) => {
        isPlacingColumn = v;
    });
    ...
    function onImportImage() {
        ...
        input.onchange = async () => {
            ...
            if (file.size > 5 * 1024 * 1024) {
                alert(
                    "Warning: Image is larger than 5MB. This may slow down the application.",
                );
            }
    ...
    function confirmImport() {
        ...
        try {
            ...
            const projectName = importFileName
                ? importFileName.replace(/\.(json|zip)$/i, "")
                : "RoomPlan Import";
    ...
    function updateHoverPos(e: MouseEvent) {
        ...
        const x =
            sidebarRight + tooltipW + 8 < viewportW
                ? sidebarRight + 8
                : -tooltipW - 8;
        // Vertically align near the mouse, clamped to viewport
        const y = Math.min(
            Math.max(e.clientY - 40, 8),
...
<div
    class="w-64 bg-white border-r border-gray-200 flex flex-col h-full overflow-hidden"
>
    <!-- Tabs -->
    <div class="flex border-b border-gray-200">
        <button
            class="flex-1 py-2.5 text-xs font-semibold uppercase tracking-wide {activeTab ===
            'draw'
                ? 'text-slate-800 border-b-2 border-blue-500 bg-blue-50'
                : 'text-gray-500 hover:text-gray-700'}"
            onclick={() => (activeTab = "draw")}>Build</button
        >
        <button
            class="flex-1 py-2.5 text-xs font-semibold uppercase tracking-wide {activeTab ===
            'rooms'
                ? 'text-slate-800 border-b-2 border-blue-500 bg-blue-50'
                : 'text-gray-500 hover:text-gray-700'}"
            onclick={() => (activeTab = "rooms")}>Rooms</button
        >
        <button
            class="flex-1 py-2.5 text-xs font-semibold uppercase tracking-wide {activeTab ===
            'objects'
                ? 'text-slate-800 border-b-2 border-blue-500 bg-blue-50'
                : 'text-gray-500 hover:text-gray-700'}"
            onclick={() => (activeTab = "objects")}>Objects</button
        >
    </div>
    ...
    <div class="flex-1 overflow-y-auto p-3">
        {#if activeTab === "draw"}
            <div class="space-y-1">
                ...
                <button
                    class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors {currentTool ===
                    'select'
                        ? 'bg-blue-50 text-slate-800 ring-1 ring-blue-200'
                        : 'hover:bg-gray-50 text-gray-700'}"
                    onclick={() => setTool("select")}
                >
                    <div
                        class="w-9 h-9 rounded-lg bg-gray-100 flex items-center justify-center {currentTool ===
                        'select'
                            ? 'bg-blue-100'
                            : ''}"
                    >
                    ...
                    <div class="text-left">
                        <div class="font-medium">
                            Select <span class="text-gray-400 text-xs ml-1"
                                >V</span
                            >
                        </div>
                    ...
                    >
                        <svg
                            ...
                            /><line x1="7" y1="8" x2="7" y2="16" /><line
                                ...
                                x2="12"
                                y2="16"
                            /><line x1="17" y1="8" x2="17" y2="16" /></svg
                        >
                    </div>
                    <div class="text-left">
                        <div class="font-medium">
                            Draw Wall <span class="text-gray-400 text-xs ml-1"
                                >W</span
                            >
                        </div>
                        <div class="text-xs text-gray-400">
                            Click to draw, dbl-click to finish
                        </div>
                    </div>
                </button>

                <h3
                    class="text-xs font-semibold text-gray-400 uppercase mb-2 mt-3"
                >
                    Structure
                </h3>
                <button
                    class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors {isPlacingStair
                        ? 'bg-blue-50 text-slate-800 ring-1 ring-blue-200'
                        : 'hover:bg-gray-50 text-gray-700'}"
                    onclick={onPlaceStair}
                >
                    <div
                        class="w-9 h-9 rounded-lg bg-gray-100 flex items-center justify-center {isPlacingStair
                            ? 'bg-blue-100'
                            : ''}"
                    >
                        <svg
                            ...
                            stroke-linejoin="round"
                            ><path
                                d="M22 5h-5V2h-3v6h-4V5H7v6H2v3h5v3h3v-3h4v3h3v-6h5z"
                            /></svg
                        >
                    </div>
                    <div class="text-left">
                ...
                <div class="flex gap-2">
                    <button
                        class="flex-1 flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm transition-colors {isPlacingColumn
                            ? 'bg-blue-50 text-slate-800 ring-1 ring-blue-200'
                            : 'hover:bg-gray-50 text-gray-700'}"
                        onclick={() => onPlaceColumn("round")}
                    >
                        <div
                            class="w-9 h-9 rounded-lg bg-gray-100 flex items-center justify-center {isPlacingColumn
                                ? 'bg-blue-100'
                                : ''}"
                        >
                            <svg
                    ...
                    >
                        <svg
                            ...
                            /><line x1="16" y1="5" x2="22" y2="5" /><line
                                ...
                                x2="19"
                                y2="8"
                            /><line x1="3" y1="12" x2="12" y2="12" /></svg
                        >
                    </div>
                    <div class="text-left">
                        <div class="font-medium">Dimension</div>
                        <div class="text-xs text-gray-400">
                            Add dimension annotations (N)
                        </div>
                    </div>
                </button>
                <button
                ...
                >
                    ...
                    >
                        <svg
                            ...
                            stroke-linecap="round"
                            stroke-linejoin="round"
                            ><path d="M2 12h5l2-7 4 14 2-7h7" /></svg
                        >
                    </div>
                    <div class="text-left">
                ...
                {#if constructionOpen}
                    <div class="grid grid-cols-2 gap-2 mb-3">
                        {#each doorCatalog as dc}
                            <button
                                class="flex flex-col items-center gap-1 p-2.5 rounded-lg border-2 transition-colors cursor-grab active:cursor-grabbing {currentTool ===
                                    'door' && selectedDoorType === dc.type
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        {#each windowCatalog as wc}
                            <button
                                class="flex flex-col items-center gap-1 p-2.5 rounded-lg border-2 transition-colors cursor-grab active:cursor-grabbing {currentTool ===
                                    'window' && selectedWindowType === wc.type
                                    ? 'border-blue-400 bg-blue-50'
                                    : 'border-gray-100 hover:border-gray-200'}"
                            ...
                            >
                                <div
                                    class="w-9 h-9 rounded-lg bg-cyan-50 flex items-center justify-center"
                                >
                                    <svg
                                        width="18"
                                        height="18"
        ...
        {:else if activeTab === "rooms"}
            <div class="space-y-2">
                ...
                <p class="text-xs text-gray-400 mb-3">
                    Click to add a room shape to the canvas
                </p>
                <div class="grid grid-cols-2 gap-2">
                    {#each roomPresets as preset}
                        ...
                        >
                            <div
                                class="w-12 h-12 rounded-lg bg-gray-50 flex items-center justify-center text-2xl font-mono"
                            >
                                {preset.icon}
                            </div>
                            <span class="text-xs font-medium text-gray-600"
                ...
                <p class="text-xs text-gray-400 mb-3">
                    Pre-furnished rooms — walls + furniture in one click
                </p>
                <div class="grid grid-cols-2 gap-2">
                    {#each roomTemplates as tmpl}
                        <button
                            ...
                            onclick={() =>
                                onPresetClick(tmpl.presetId, tmpl.name)}
                            draggable="true"
                            ondragstart={(e) => {
                                e.dataTransfer?.setData(
                                    "application/o3d-type",
                        ...
                        >
                            <div
                                class="w-12 h-12 rounded-lg bg-green-50 flex items-center justify-center text-lg"
                            >
                            ...
                            >
                            <span class="text-[10px] text-gray-400"
                                >{tmpl.furniture.length} items</span
                            >
                        </button>
                    {/each}
                </div>
        ...
        {:else if activeTab === "objects"}
            <div class="space-y-2">
                ...
                <div class="flex flex-wrap gap-1 max-h-24 overflow-y-auto">
                    <button
                        class="px-2 py-0.5 rounded-full text-[10px] font-medium {selectedCategory ===
                        'All'
                            ? 'bg-blue-600 text-white'
                            : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}"
                        onclick={() => (selectedCategory = "All")}>All</button
                    >
                    <button
                        class="px-2 py-0.5 rounded-full text-[10px] font-medium {selectedCategory ===
                        'Favorites'
                    ...
                    {#each furnitureCategories as cat}
                        <button
                            ...
                            style={selectedCategory === cat
                                ? `background-color: ${categoryColors[cat] ?? "#6b7280"}`
                ...
                {#if !search && selectedCategory === "All" && recentItems.length > 0}
                    <div class="mt-1">
                        <h4
                            class="text-[10px] font-semibold text-gray-400 uppercase mb-1.5"
                        >
                            Recent
                        </h4>
                        <div class="grid grid-cols-2 gap-2">
                            {#each recentItems as item}
                                <button
                                    class="relative flex flex-col items-center gap-1 p-2.5 rounded-lg border-2 transition-colors cursor-grab active:cursor-grabbing {currentPlacing ===
                                    item.id
                                ...
                                >
                                    ...
                                    <span
                                        role="button"
                                        tabindex="0"
                                        class="absolute top-1 right-1 text-[12px] leading-none cursor-pointer {favoriteIds.includes(
                                            item.id,
                                        )
                                            ? 'text-pink-500'
                                        ...
                                        title={favoriteIds.includes(item.id)
                                    ...
                                    <span
                                        class="text-[10px] font-medium text-gray-600 leading-tight text-center"
                                        >{item.name}</span
                                    >
                                </button>
                            {/each}
                ...
                <div class="grid grid-cols-2 gap-2 mt-2">
                    {#each filtered as item}
                        {@const s = search.toLowerCase()}
                        <button
                            class="relative flex flex-col items-center gap-1 p-3 rounded-lg border-2 transition-colors cursor-grab active:cursor-grabbing {currentPlacing ===
                            item.id
                                ? 'border-blue-400 bg-blue-50 ring-1 ring-blue-300'
                                : 'border-gray-100 hover:border-blue-300 hover:bg-blue-50'}"
                            onclick={() => onFurnitureClick(item)}
                        ...
                        >
                            ...
                            {#if s && item.name.toLowerCase().includes(s)}
                                {@const idx = item.name
                                    ...
                                    .indexOf(s)}
                                <span class="text-xs font-medium text-gray-600"
                                    >{item.name.slice(0, idx)}<mark
                                        class="bg-yellow-200 text-gray-800 rounded-sm px-0.5"
                                        >{item.name.slice(
                                            idx,
                                            idx + s.length,
                                        )}</mark
                                    >{item.name.slice(idx + s.length)}</span
                                >
                            {:else}
                                <span class="text-xs font-medium text-gray-600"
                                    >{item.name}</span
...
{#if showPreview && hoveredItem}
    ...
    >
        ...
        >
            <div
                class="w-full h-[120px] bg-gray-50 flex items-center justify-center p-3"
            >
                <div
...
{#if showImportDialog}
    ...
    >
        <div
            class="bg-white rounded-xl shadow-2xl w-80 p-5"
        ...
        >
            ...
            <div class="space-y-3">
                <label class="flex items-start gap-2.5 cursor-pointer">
                    ...
                    <div>
                        ...
                        <div class="text-xs text-gray-400">
                            Snap near-horizontal/vertical walls to axis
                        </div>
                    </div>
                </label>
                ...
                <label class="flex items-start gap-2.5 cursor-pointer">
                    <input
                        type="checkbox"
                        bind:checked={optOrthogonal}
                        class="accent-blue-500 mt-0.5"
                    />
                    <div>
                        <div class="text-sm font-medium text-gray-700">
                            Enforce orthogonal <span
                                class="text-xs text-blue-400 font-mono"
                                >{ORTHO_VERSION}</span
                            >
                        </div>
                        <div class="text-xs text-gray-400">
                            Force all walls to 90°/180° angles
                        </div>
                    </div>
                </label>
                ...
                <label class="block">
                    <div class="text-xs text-gray-500 mb-1">
                        Corner merge distance (cm)
                    </div>
                    <input
                        type="number"
                        bind:value={optMergeDistance}
                        min="0"
                        max="50"
                        step="5"
            ...
            <div class="flex gap-2 mt-5">
                <button
                    onclick={cancelImport}
                    class="flex-1 px-3 py-2 border border-gray-200 rounded-lg text-sm text-gray-600 hover:bg-gray-50 transition-colors"
                    >Cancel</button
                >
                <button
