<script lang="ts">
    ...
    function getCS(): CanvasState {
        return { ctx, width, height, zoom, camX, camY };
    }
    ...
    $effect(() => {
        showFurnitureStore.set(layerVis.furniture);
    });
    ...
    let roomDragStartPositions: Map<string, { start: Point; end: Point }> =
        new Map();

    // Wall endpoint drag state (includes all connected walls at the corner)
    let draggingWallEndpoint: {
        wallId: string;
        endpoint: "start" | "end";
    } | null = $state(null);
    let draggingConnectedEndpoints: {
        wallId: string;
        endpoint: "start" | "end";
    }[] = $state([]);
    let dragPreview: {
    ...
    let draggingWallParallel: {
        wallId: string;
        startMousePos: Point;
        origStart: Point;
        origEnd: Point;
        origCurve?: Point;
        connectedStart: { wallId: string; endpoint: "start" | "end" }[];
        connectedEnd: { wallId: string; endpoint: "start" | "end" }[];
    } | null = $state(null);
    ...
    let draggingMultiSelect: {
        startMousePos: Point;
        origPositions: Map<
            string,
            { start?: Point; end?: Point; position?: Point }
        >;
    } | null = $state(null);

    // Clipboard for copy/paste (Ctrl+C / Ctrl+V)
    let clipboard: {
        items: Array<{ type: "furniture" | "door" | "window"; data: any }>;
    } | null = $state(null);
    ...
    function getMultiSelectBBox(): {
        ...
        for (const id of currentSelectedIds) {
            ...
            if (fi) {
                expand(fi.position.x, fi.position.y);
                continue;
            }
            if (currentFloor!.stairs) {
                const st = currentFloor!.stairs.find((s) => s.id === id);
                if (st) {
                    expand(st.position.x, st.position.y);
                    continue;
                }
            }
            if (currentFloor!.columns) {
                ...
                if (col) {
                    expand(col.position.x, col.position.y);
                    continue;
                }
            }
            ...
            if (door) {
                const w = currentFloor!.walls.find((w) => w.id === door.wallId);
                if (w) {
                    const cx =
                        w.start.x + (w.end.x - w.start.x) * door.position;
                    const cy =
                        w.start.y + (w.end.y - w.start.y) * door.position;
                    expand(cx, cy);
                }
                continue;
            }
    ...
    function snapFurnitureToWall(
        ...
        for (const wall of currentFloor.walls) {
            ...
            const ux = wx / wLen,
                uy = wy / wLen;
            const nx = -uy,
                ny = ux; // normal pointing "left" of wall direction
            ...
            if (along < -cat.width / 2 || along > wLen + cat.width / 2)
                continue;
            ...
            if (snapDist < bestDist) {
                ...
                const clampedAlong = Math.max(
                    cat.width / 2,
                    Math.min(wLen - cat.width / 2, along),
                );
    ...
    function findConnectedEndpoints(
        pt: Point,
    ...
    function magneticSnap(
        ...
        if (!best.snappedToEndpoint) {
            ...
            for (const w of currentFloor.walls) {
                ...
                const t =
                    ((p.x - w.start.x) * wx + (p.y - w.start.y) * wy) /
                    (wLen * wLen);
    ...
    function angleSnap(start: Point, end: Point): Point {
        ...
        for (const sa of snapAngles) {
            if (Math.abs(angle - sa) < ANGLE_THRESHOLD) {
                return {
                    x: start.x + len * Math.cos(sa),
                    y: start.y + len * Math.sin(sa),
                };
            }
        }
        return end;
    ...
    function wallLength(w: Wall): number {
        if (w.curvePoint) {
            ...
            for (let i = 1; i <= N; i++) {
                ...
                const nx =
                    mt * mt * w.start.x +
                    2 * mt * t * w.curvePoint.x +
                    t * t * w.end.x;
                const ny =
                    mt * mt * w.start.y +
                    2 * mt * t * w.curvePoint.y +
                    t * t * w.end.y;
    ...
    function drawFurniturePreview() {
        ...
        const wallSnap = snapFurnitureToWall(
            mousePos,
            currentPlacingId,
            currentPlacingRotation,
        );
        ...
        if (wallSnap && currentFloor) {
            const snapWall = currentFloor.walls.find(
                (wl) => wl.id === wallSnap.wallId,
            );
            if (snapWall) {
    ...
    function drawPlacementPreview() {
        ...
        const wall = currentFloor.walls.find(
            (w) => w.id === placementPreview!.wallId,
        );
        if (!wall) return;
        ...
        if (isDoor) {
            ...
            ctx.lineTo(
                hingeX + r * Math.cos(endAngle),
                hingeY + r * Math.sin(endAngle),
            );
            ctx.stroke();
            ctx.lineWidth = 1.5;
            ctx.strokeStyle = "#3b82f6";
    ...
    function drawAngleGuides(start: Point) {
        ...
        for (const a of angles) {
            ...
            ctx.moveTo(s.x, s.y);
            ctx.lineTo(
                s.x + guideLen * Math.cos(a),
                s.y + guideLen * Math.sin(a),
            );
            ctx.stroke();
        }
        ctx.setLineDash([]);
    ...
    function updateDetectedRooms() {
        ...
        const hash = JSON.stringify(
            currentFloor.walls.map((w) => [w.start, w.end]),
        );
        ...
        for (const nr of newRooms) {
            ...
            const existing = detectedRooms.find((old) => {
                ...
                return (
                    oldWalls.size === nrWalls.size &&
                    [...nrWalls].every((w) => oldWalls.has(w))
                );
            });
            if (existing) {
                nr.id = existing.id;
                ...
                const saved = savedRooms.find((sr) => {
                    ...
                    return (
                        srWalls.size === nrWalls.size &&
                        [...nrWalls].every((w) => srWalls.has(w))
                    );
                });
                if (saved) {
                    nr.id = saved.id;
                    nr.name = saved.name;
                    if (saved.floorTexture)
                        nr.floorTexture = saved.floorTexture;
                }
            }
        }
    ...
    function drawRulers() {
        ...
        if (isImperial) {
            ...
            const inchCm = 2.54;
            const niceInchSteps = [
                1, 2, 6, 12, 24, 60, 120, 240, 600, 1200, 2400,
            ];
        ...
        for (let wx = startTick; wx <= worldRight; wx += minorStep) {
            ...
            if (isMajor) {
                ...
                const label = isOrigin
                    ? "0"
                    : rulerLabel(wx, tickStep, isImperial);
        ...
        for (let wy = startTickY; wy <= worldBottom; wy += minorStep) {
            ...
            if (isMajor) {
                const label = isOrigin
                    ? "0"
                    : rulerLabel(wy, tickStep, isImperial);
                ctx.save();
    ...
    function draw() {
        ...
        if (showDoors) {
            for (const d of floor.doors) {
                ...
                if (wall) {
                    ...
                    if (showDimensions && isSelected(d.id))
                        drawDoorDistanceDimensions(wall, d);
                }
            }
        }
        if (showWindows) {
            for (const win of floor.windows) {
                ...
                if (wall) {
                    ...
                    if (showDimensions && isSelected(win.id))
                        drawWindowDistanceDimensions(wall, win);
                }
            }
        }
        ...
        if (showFurniture) {
            for (const fi of floor.furniture) {
                ...
                if (selected && draggingFurnitureId === fi.id)
                    drawAlignmentGuides(fi);
        ...
        if (
            showDimensions &&
            dimSettings.showObjectDistance &&
            currentSelectedId &&
            showFurniture
        ) {
            const selFurniture = floor.furniture.find(
            ...
            if (selFurniture) {
                ...
                if (cat) {
                    const fw =
                        (selFurniture.width ?? cat.width) *
                        Math.abs(selFurniture.scale?.x ?? 1);
                    const fd =
                        (selFurniture.depth ?? cat.depth) *
                        Math.abs(selFurniture.scale?.y ?? 1);
                    ...
                    const otherFurniture = floor.furniture.filter(
                        (f) => f.id !== selFurniture.id,
                    );
                    // Track closest furniture per direction
                    const closestFurn: Record<
                        string,
                        { dist: number; dim: DimLine }
                    > = {};
                    ...
                    for (const other of otherFurniture) {
                        ...
                        if (!oCat) continue;
                        const ow =
                            (other.width ?? oCat.width) *
                            Math.abs(other.scale?.x ?? 1);
                        const od =
                            (other.depth ?? oCat.depth) *
                            Math.abs(other.scale?.y ?? 1);
                        ...
                        const midY =
                            Math.max(fTop, oTop) / 2 +
                            Math.min(fBottom, oBottom) / 2;
                        const midX =
                            Math.max(fLeft, oLeft) / 2 +
                            Math.min(fRight, oRight) / 2;
                    ...
                    for (const dir of dirs) {
                        const wallDim = allDimensions.find(
                            (d) => d.dir === dir,
                        );
                        ...
                        if (furnDim && wallDim) {
                            ...
                            const wallDist = Math.hypot(
                                wallDim.to.x - wallDim.from.x,
                                wallDim.to.y - wallDim.from.y,
                            );
        ...
        if (isPlacingColumn) {
            ctx.save();
            ctx.globalAlpha = 0.5;
            const preview: Column = {
                id: "preview",
                position: mousePos,
                rotation: 0,
                shape: placingColShape,
                diameter: 30,
                height: 280,
                color: "#cccccc",
            };
        ...
        if (
            ...
            wallStart &&
            currentTool === "wall" &&
            (wallStart.x !== wallSequenceFirst.x ||
                wallStart.y !== wallSequenceFirst.y)
        ) {
            const fp = worldToScreen(wallSequenceFirst.x, wallSequenceFirst.y);
            const distToFirst = Math.hypot(
                mousePos.x - wallSequenceFirst.x,
                mousePos.y - wallSequenceFirst.y,
            );
        ...
        if (wallStart && currentTool === "wall") {
            ...
            if (shiftDown) {
                ...
                if (slen > 5) {
                    ...
                    endPt = {
                        x: wallStart.x + slen * Math.cos(bestAngle),
                        y: wallStart.y + slen * Math.sin(bestAngle),
                    };
                }
            } else {
            ...
            const angle =
                (Math.atan2(endPt.y - wallStart.y, endPt.x - wallStart.x) *
                    180) /
                Math.PI;
            ...
            if (
                (endPt as any).snappedToWall &&
                (endPt as any).snappedWallId &&
                currentFloor
            ) {
                ...
                const targetWall = currentFloor.walls.find(
                    (w) => w.id === (endPt as any).snappedWallId,
                );
                if (targetWall) {
                    const tw1 = worldToScreen(
                        targetWall.start.x,
                        targetWall.start.y,
                    );
                    const tw2 = worldToScreen(
                        targetWall.end.x,
                        targetWall.end.y,
                    );
                    ctx.strokeStyle = "#ec4899";
        ...
        if (draggingHandle === "rotate" && currentSelectedId && currentFloor) {
            const fi = currentFloor.furniture.find(
                (f) => f.id === currentSelectedId,
            );
            if (fi) {
    ...
    onMount(() => {
        ...
        setTextureLoadCallback(() => {
            /* draw loop is already running via rAF */
        });
        ...
        const unsub3 = selectedRoomId.subscribe((id) => {
            currentSelectedRoomId = id;
            markDirty();
        });
        const unsub4 = placingFurnitureId.subscribe((id) => {
            currentPlacingId = id;
            markDirty();
        });
        const unsub5 = placingRotation.subscribe((r) => {
            currentPlacingRotation = r;
            markDirty();
        });
        const unsub7 = detectedRoomsStore.subscribe((rooms) => {
            if (rooms.length > 0) detectedRooms = rooms;
            markDirty();
        });
        const unsub8 = placingDoorType.subscribe((t) => {
            currentDoorType = t;
            markDirty();
        });
        const unsub9 = placingWindowType.subscribe((t) => {
            currentWindowType = t;
            markDirty();
        });
        const unsub10 = snapEnabled.subscribe((v) => {
            currentSnapEnabled = v;
            markDirty();
        });
        const unsub_snapgrid = projectSettings.subscribe((s) => {
            currentSnapToGrid = s.snapToGrid;
            currentGridSize = s.gridSize;
            markDirty();
        });
        const unsub11 = placingStair.subscribe((v) => {
            isPlacingStair = v;
            markDirty();
        });
        ...
        const unsub_col = placingColumn.subscribe((v) => {
            isPlacingColumn = v;
            markDirty();
        });
        const unsub_cols = placingColumnShape.subscribe((v) => {
            placingColShape = v;
            markDirty();
        });
        const unsub12 = calibrationMode.subscribe((v) => {
            isCalibrating = v;
            markDirty();
        });
        const unsub13 = calibrationPoints.subscribe((pts) => {
            calPoints = pts;
            markDirty();
        });
        const unsub_multi = selectedElementIds.subscribe((ids) => {
            currentSelectedIds = ids;
            markDirty();
        });
        const unsub14 = activeFloor.subscribe((f) => {
            if (
                f?.backgroundImage?.dataUrl &&
                (!bgImage || bgImage.src !== f.backgroundImage.dataUrl)
            ) {
                const img = new Image();
                img.onload = () => {
                    bgImage = img;
    ...
    function getWorldBBox(): {
        ...
        for (const fi of currentFloor.furniture) {
            ...
            if (!cat) continue;
            const r = Math.hypot(
                (fi.width ?? cat.width) / 2,
                (fi.depth ?? cat.depth) / 2,
            );
        ...
        if (currentFloor.stairs)
            for (const st of currentFloor.stairs) {
                expand(
                    st.position.x - st.width / 2,
                    st.position.y - st.depth / 2,
                );
                expand(
                    st.position.x + st.width / 2,
                    st.position.y + st.depth / 2,
    ...
    function findRoomLabelAt(p: Point): Room | null {
        ...
        for (const room of detectedRooms) {
            ...
            if (Math.abs(p.x - lx) < hitW && Math.abs(p.y - ly) < hitH) {
                ...
                if (
                    room.labelOffset &&
                    (room.labelOffset.x !== 0 || room.labelOffset.y !== 0)
                ) {
                    const resetOffX = 50 / zoom; // approximate reset icon position
                    if (
                        p.x > lx + resetOffX * 0.5 &&
                        Math.abs(p.y - ly) < 15 / zoom
                    ) {
        ...
        if (tool === "select" && currentFloor?.guides) {
            ...
            for (const g of currentFloor.guides) {
                if (
                    ...
                    Math.abs(wp.y - g.position) < GUIDE_HIT
                ) {
                ...
                if (
                    ...
                    Math.abs(wp.x - g.position) < GUIDE_HIT
                ) {
                    selectedGuideId = g.id;
                    draggingGuideId = g.id;
                    selectedElementId.set(null);
                ...
                if (newPts.length >= 2) {
                    ...
                    if (realDist && Number(realDist) > 0) {
                        ...
                        if (currentFloor?.backgroundImage) {
                            updateBackgroundImage({
                                scale:
                                    currentFloor.backgroundImage.scale *
                                    (1 / pixelsPerCm),
                            });
                        }
                    }
        ...
        if (tool === "wall") {
            ...
            if (!wallStart) {
                ...
                if (
                    wallSequenceFirst &&
                    Math.hypot(
                        endPt.x - wallSequenceFirst.x,
                        endPt.y - wallSequenceFirst.y,
                    ) < 20 &&
                    Math.hypot(
                        wallStart.x - wallSequenceFirst.x,
                        wallStart.y - wallSequenceFirst.y,
                    ...
                    Math.hypot(endPt.x - wallStart.x, endPt.y - wallStart.y) > 5
                ) {
            ...
            if (currentSelectedIds.size >= 2 && currentFloor) {
                ...
                if (
                    ...
                    const origPositions = new SvelteMap<
                        string,
                        { start?: Point; end?: Point; position?: Point }
                    >();
                    for (const id of currentSelectedIds) {
                        ...
                        if (currentFloor.stairs) {
                            const st = currentFloor.stairs.find(
                                (s) => s.id === id,
                            );
                            if (st) {
                                origPositions.set(id, {
                        ...
                        if (currentFloor.columns) {
                            const col = currentFloor.columns.find(
                                (c) => c.id === id,
                            );
                            if (col) {
                                origPositions.set(id, {
            ...
            if (currentSelectedId && currentFloor) {
                const selWall = currentFloor.walls.find(
                ...
                if (selWall) {
                    ...
                    if (
                        Math.hypot(
                            wp.x - selWall.start.x,
                            wp.y - selWall.start.y,
                        ) < epThreshold
                    ) {
                        draggingWallEndpoint = {
                            wallId: selWall.id,
                        ...
                        draggingConnectedEndpoints = findConnectedEndpoints(
                            selWall.start,
                            selWall.id,
                        );
                    ...
                    if (
                        Math.hypot(wp.x - selWall.end.x, wp.y - selWall.end.y) <
                        epThreshold
                    ) {
                        draggingWallEndpoint = {
                            wallId: selWall.id,
                    ...
                    const curveHandlePt = selWall.curvePoint
                        ? selWall.curvePoint
                        : {
                              x: (selWall.start.x + selWall.end.x) / 2,
                              y: (selWall.start.y + selWall.end.y) / 2,
                          };
                    if (
                        Math.hypot(
                            wp.x - curveHandlePt.x,
                            wp.y - curveHandlePt.y,
                        ) < epThreshold
                    ) {
                        if (e.altKey) {
                            ...
                            draggingWallParallel = {
                                ...
                                origEnd: { ...selWall.end },
                                connectedStart: findConnectedEndpoints(
                                    selWall.start,
                                    selWall.id,
                                ),
                                connectedEnd: findConnectedEndpoints(
                                    selWall.end,
                                    selWall.id,
                                ),
                            };
            ...
            function selectElement(
                ...
                if (isShift) {
                    ...
                    const group = currentFloor
                        ? findGroupForElement(currentFloor, id)
                        : undefined;
                    if (group && !isCtrl) {
                        selectedElementId.set(id);
            ...
            if (col) {
                ...
                if (!e.shiftKey) {
                    ...
                    columnDragOffset = {
                        x: wp.x - col.position.x,
                        y: wp.y - col.position.y,
                    };
            ...
            if (stair) {
                ...
                if (!e.shiftKey) {
                    ...
                    stairDragOffset = {
                        x: wp.x - stair.position.x,
                        y: wp.y - stair.position.y,
                    };
                    commitFurnitureMove(); // snapshot before drag for undo
                }
                return;
            ...
            if (fi) {
                ...
                if (!e.shiftKey && !fi.locked) {
                    ...
                    commitFurnitureMove(); // snapshot before drag for undo
                    dragOffset = {
                        x: wp.x - fi.position.x,
                        y: wp.y - fi.position.y,
                    };
            ...
            if (wall) {
                ...
                if (labelRoom) {
                    ...
                    roomLabelOrigOffset = {
                        x: labelRoom.labelOffset?.x ?? 0,
                        y: labelRoom.labelOffset?.y ?? 0,
                    };
                ...
                if (room) {
                    ...
                    for (const wid of room.walls) {
                        const w = currentFloor!.walls.find(
                            (wall) => wall.id === wid,
                        );
                        if (w)
                            roomDragStartPositions.set(wid, {
                                start: { ...w.start },
                                end: { ...w.end },
                            });
                    }
    ...
    function onMouseMove(e: MouseEvent) {
        ...
        if (draggingRoomLabelId) {
            ...
            const newOffset = {
                x: roomLabelOrigOffset.x + dx,
                y: roomLabelOrigOffset.y + dy,
            };
            detectedRoomsStore.update((rooms) =>
                rooms.map((r) =>
                    r.id === draggingRoomLabelId
                        ? { ...r, labelOffset: newOffset }
                        : r,
                ),
            );
            return;
        ...
        if (draggingGuideId && currentFloor?.guides) {
            ...
            if (g) {
                const newPos =
                    g.orientation === "horizontal" ? mousePos.y : mousePos.x;
        ...
        if (draggingWallEndpoint) {
            ...
            const excludeIds = new Set<string>([
                draggingWallEndpoint.wallId,
                ...draggingConnectedEndpoints.map((c) => c.wallId),
            ]);
            let pt = magneticSnap(mousePos, excludeIds);
            // Angle snap to the opposite endpoint of the primary wall being dragged
            if (currentFloor) {
                const wall = currentFloor.walls.find(
                    (w) => w.id === draggingWallEndpoint!.wallId,
                );
                if (wall) {
                    const other =
                        draggingWallEndpoint.endpoint === "start"
                            ? wall.end
                            : wall.start;
        ...
        if (draggingWallParallel && currentFloor) {
            const wall = currentFloor.walls.find(
                (w) => w.id === draggingWallParallel!.wallId,
            );
            if (wall) {
        ...
        if (draggingMultiSelect && currentFloor) {
            ...
            const dx =
                Math.round(
                    (mousePos.x - draggingMultiSelect.startMousePos.x) /
                        mSnapStep,
                ) * mSnapStep;
            const dy =
                Math.round(
                    (mousePos.y - draggingMultiSelect.startMousePos.y) /
                        mSnapStep,
                ) * mSnapStep;
            for (const [id, orig] of draggingMultiSelect.origPositions) {
                if (orig.start && orig.end) {
                    // Wall — move both endpoints
                    moveWallEndpoint(id, "start", {
                        x: orig.start.x + dx,
                        y: orig.start.y + dy,
                    });
                    moveWallEndpoint(id, "end", {
                        x: orig.end.x + dx,
                        y: orig.end.y + dy,
                    });
                    ...
                    const newPos = {
                        x: orig.position.x + dx,
                        y: orig.position.y + dy,
                    };
                    const fi = currentFloor.furniture.find((f) => f.id === id);
                    if (fi) {
        ...
        if (draggingRoomId && currentFloor && roomDragStartPositions.size > 0) {
            ...
            const dx =
                Math.round((mousePos.x - roomDragStartMouse.x) / rSnapStep) *
                rSnapStep;
            const dy =
                Math.round((mousePos.y - roomDragStartMouse.y) / rSnapStep) *
                rSnapStep;
            for (const [wid, orig] of roomDragStartPositions) {
                moveWallEndpoint(wid, "start", {
                    x: orig.start.x + dx,
                    y: orig.start.y + dy,
                });
                moveWallEndpoint(wid, "end", {
                    x: orig.end.x + dx,
                    y: orig.end.y + dy,
                });
            }
        ...
        if (draggingCurveHandle && currentFloor) {
            ...
            if (wall) {
                ...
                if (distToMid < 5) {
                    ...
                    updateWall(draggingCurveHandle, {
                        curvePoint: {
                            x: snap(mousePos.x),
                            y: snap(mousePos.y),
                        },
                ...
                if (cat) {
                    if (draggingHandle === "rotate") {
                        ...
                        const isEdge = [
                            ...
                            "resize-l",
                            "resize-r",
                        ].includes(draggingHandle);
                        const resizesX =
                            !isEdge ||
                            draggingHandle === "resize-l" ||
                            draggingHandle === "resize-r";
                        const resizesY =
                            !isEdge ||
                            draggingHandle === "resize-t" ||
                            draggingHandle === "resize-b";
                        if (resizesX) {
                            newSx = Math.abs(localX * 2) / cat.width;
                            newSx = Math.max(
                                minScale,
                                Math.round(newSx * 20) / 20,
                            );
                        }
                        if (resizesY) {
                            ...
                            newSy = Math.max(
                                minScale,
                                Math.round(newSy * 20) / 20,
                            );
                        }
                        // Shift: maintain aspect ratio
                        if (shiftDown && resizesX && resizesY) {
                            const origRatio =
                                (handleOrigScale.x * cat.width) /
                                (handleOrigScale.y * cat.depth);
                            const currentRatio =
                                (newSx * cat.width) / (newSy * cat.depth);
                            if (currentRatio > origRatio) {
                                ...
                                newSx =
                                    (newSy * cat.depth * origRatio) / cat.width;
                            }
                        }
                        scaleFurniture(currentSelectedId, {
                            x: newSx,
                            y: newSy,
                        });
                    }
        ...
        if (draggingColumnId && currentFloor?.columns) {
            const basePos = {
                x: mousePos.x - columnDragOffset.x,
                y: mousePos.y - columnDragOffset.y,
            };
            moveColumn(draggingColumnId, {
                x: snap(basePos.x),
                y: snap(basePos.y),
            });
        }
        if (draggingStairId && currentFloor?.stairs) {
            const basePos = {
                x: mousePos.x - stairDragOffset.x,
                y: mousePos.y - stairDragOffset.y,
            };
            moveStair(draggingStairId, {
                x: snap(basePos.x),
                y: snap(basePos.y),
            });
        }
                ...
                if (wallSnap) {
                    ...
                    if (currentFloor?.guides) {
                        for (const g of currentFloor.guides) {
                            if (
                                g.orientation === "horizontal" &&
                                Math.abs(snapped.y - g.position) < GUIDE_SNAP
                            ) {
                                snapped.y = g.position;
                            }
                            if (
                                g.orientation === "vertical" &&
                                Math.abs(snapped.x - g.position) < GUIDE_SNAP
                            ) {
        ...
        if (draggingDoorId && currentFloor) {
            const door = currentFloor.doors.find(
                (d) => d.id === draggingDoorId,
            );
            if (door) {
                const wall = currentFloor.walls.find(
                    (w) => w.id === door.wallId,
        ...
        if (draggingWindowId && currentFloor) {
            const win = currentFloor.windows.find(
                (w) => w.id === draggingWindowId,
            );
            if (win) {
                const wall = currentFloor.walls.find(
                    (w) => w.id === win.wallId,
        ...
        if (
            ...
            if (wall) {
                placementPreview = {
                    wallId: wall.id,
                    position: positionOnWall(mousePos, wall),
                    type: currentTool as "door" | "window",
                };
            } else {
        ...
        if (marqueeStart && marqueeEnd && currentFloor) {
            ...
            if (marqueeW > 5 || marqueeH > 5) {
                ...
                for (const d of currentFloor.doors) {
                    ...
                    if (w) {
                        const cx =
                            w.start.x + (w.end.x - w.start.x) * d.position;
                        const cy =
                            w.start.y + (w.end.y - w.start.y) * d.position;
                ...
                for (const win of currentFloor.windows) {
                    ...
                    if (w) {
                        const cx =
                            w.start.x + (w.end.x - w.start.x) * win.position;
                        const cy =
                            w.start.y + (w.end.y - w.start.y) * win.position;
    ...
    function onKeyDown(e: KeyboardEvent) {
        ...
        if ((e.ctrlKey || e.metaKey) && e.key === "a" && !e.shiftKey) {
            ...
            if (currentFloor) {
                ...
                if (currentFloor.stairs)
                    for (const s of currentFloor.stairs) allIds.add(s.id);
                if (currentFloor.columns)
                    for (const c of currentFloor.columns) allIds.add(c.id);
        ...
        if ((e.ctrlKey || e.metaKey) && e.key === "c" && !e.shiftKey) {
            if (currentFloor) {
                ...
                for (const id of idsToCheck) {
                    const fi = currentFloor.furniture.find((f) => f.id === id);
                    if (fi) {
                        items.push({ type: "furniture", data: { ...fi } });
                        continue;
                    }
                    const door = currentFloor.doors.find((d) => d.id === id);
                    if (door) {
                        items.push({ type: "door", data: { ...door } });
                        continue;
                    }
                    const win = currentFloor.windows.find((w) => w.id === id);
                    if (win) {
                        items.push({ type: "window", data: { ...win } });
                        continue;
                    }
                }
                if (items.length > 0) {
            ...
            if (clipboard && clipboard.items.length > 0 && currentFloor) {
                ...
                for (const item of clipboard.items) {
                    ...
                    if (newId) {
                        ...
                        const newData =
                            item.type === "furniture"
                                ? currentFloor.furniture.find(
                                      (f) => f.id === newId,
                                  )
                                : item.type === "door"
                                  ? currentFloor.doors.find(
                                        (d) => d.id === newId,
                                    )
                                  : currentFloor.windows.find(
                                        (w) => w.id === newId,
                                    );
                        newItems.push({
                            type: item.type,
    ...
    function onDragOver(e: DragEvent) {
        if (e.dataTransfer?.types.includes("application/o3d-type")) {
            ...
            const itemType = e.dataTransfer?.types.includes(
                "application/o3d-type",
            )
                ? "item"
                : "";
            // Default preview size (furniture ~60x60cm, room ~400x300cm)
            const isRoom = e.dataTransfer?.types.includes(
                "application/o3d-type",
            );
            dragPreview = {
                x: wp.x,
    ...
    function handleContextMenuAction(action: string, _data?: any) {
        ...
        switch (action) {
            ...
            case "duplicate-furniture":
                if (id) {
                    const newId = duplicateFurniture(id);
            ...
            case "bring-to-front":
                if (id) {
                    const idx = currentFloor.furniture.findIndex(
                        (f) => f.id === id,
                    );
                    if (idx >= 0) {
            ...
            case "toggle-curve":
                if (id && ctxMenuWall) {
                    if (ctxMenuWall.curvePoint) {
                        ...
                        const mx =
                            (ctxMenuWall.start.x + ctxMenuWall.end.x) / 2;
                        const my =
                            (ctxMenuWall.start.y + ctxMenuWall.end.y) / 2;
            ...
            case "select-all":
                if (currentFloor) {
                    ...
                    if (currentFloor.stairs)
                        currentFloor.stairs.forEach((s) => allIds.add(s.id));
                    if (currentFloor.columns)
                        currentFloor.columns.forEach((c) => allIds.add(c.id));
                    selectedElementIds.set(allIds);
                }
                break;
            case "add-wall":
                                ...
                                : draggingHandle === "rotate"
                                  ...
                                  : draggingHandle === "resize-t" ||
                                    ...
                                    : draggingHandle === "resize-l" ||
                                        draggingHandle === "resize-r"
                                      ? "ew-resize"
                                      : draggingHandle?.startsWith("resize")
                                        ? "nwse-resize"
                                        : currentTool === "text"
                                          ? "text"
                                          : currentTool === "select"
...
<div class="w-full h-full relative overflow-hidden" role="application">
    ...
    {#if editingRoomId}
        <input
            ...
            oninput={(e) => {
                editingRoomName = (e.target as HTMLInputElement).value;
            }}
            onkeydown={(e) => {
                if (e.key === "Enter") {
    ...
    {/if}
    <!-- Empty state hint -->
    {#if currentFloor && currentFloor.walls.length === 0 && currentFloor.furniture.length === 0 && currentFloor.doors.length === 0 && currentFloor.stairs.length === 0}
        <div
        ...
        >
            <div class="text-center opacity-60">
                ...
                <div class="text-sm font-medium text-gray-500">
                    Start building your floor plan
                </div>
                <div class="text-sm text-gray-500 mt-1">
                    Draw walls with <span
                        class="font-mono bg-gray-100 px-1 rounded">W</span
                    > or drag items from the sidebar
                </div>
            </div>
        </div>
    {/if}
    ...
    {/if}
    <div
        class="absolute bottom-2 right-2 bg-white/80 rounded px-2 py-1 text-xs text-gray-500 flex gap-3"
    >
        {#if detectedRooms.length > 0}
            <span
                >{detectedRooms.length} room{detectedRooms.length !== 1
                    ? "s"
                    : ""}</span
            >
            <span
                >{formatArea(
                    detectedRooms.reduce((s, r) => s + r.area, 0),
        ...
        {#if currentFloor}
            <span
                >{currentFloor.walls.length} wall{currentFloor.walls.length !==
                1
                    ? "s"
                    : ""}</span
            >
            {#if currentFloor.doors.length > 0}
                <span
                    >{currentFloor.doors.length} door{currentFloor.doors
                        .length !== 1
                        ? "s"
                        : ""}</span
                >
            ...
            {#if currentFloor.windows.length > 0}
                <span
                    >{currentFloor.windows.length} window{currentFloor.windows
                        .length !== 1
                        ? "s"
                        : ""}</span
                >
            ...
            {#if currentFloor.furniture.length > 0}
                <span
                    >{currentFloor.furniture.length} object{currentFloor
                        .furniture.length !== 1
                        ? "s"
                        : ""}</span
                >
        ...
        {#if currentSelectedIds.size > 1}
            <span class="text-blue-600 font-medium"
                >{currentSelectedIds.size} selected</span
            >
            <span class="text-gray-300">|</span>
        {/if}
    ...
    {#if showLayerPanel}
        <div
            class="absolute bottom-12 right-2 z-20 bg-white rounded-lg shadow-lg border border-gray-200 p-3 text-xs min-w-[160px]"
        >
            ...
            {#each [["walls", "Walls"], ["doors", "Doors"], ["windows", "Windows"], ["furniture", "Furniture"], ["stairs", "Stairs"], ["columns", "Columns"], ["guides", "Guides"], ["measurements", "Measurements"]] as [key, label]}
                <label
                    class="flex items-center gap-2 py-0.5 cursor-pointer hover:bg-gray-50 rounded px-1"
                >
                    <input
                        type="checkbox"
                        checked={(layerVis as Record<string, boolean>)[key]}
                        onchange={() =>
                            layerVisibility.update((v) => ({
                                ...v,
                                [key]: !(v as Record<string, boolean>)[key],
                            }))}
    ...
    {#if (currentSelectedId || currentSelectedIds.size > 0) && currentFloor && currentTool === "select"}
        ...
        {#if el}
            <div
                ...
                style="left: {el.pos.x}px; top: {el.pos.y -
                    44}px; transform: translateX(-50%);"
            >
                <button
                    ...
                    onclick={() => {
                        ...
                        let newId: string | null = null;
                        if (el.type === "door")
                            newId = duplicateDoor(currentSelectedId);
                        else if (el.type === "window")
                            newId = duplicateWindow(currentSelectedId);
                        else if (el.type === "furniture")
                            newId = duplicateFurniture(currentSelectedId);
                        else if (el.type === "wall")
                            newId = duplicateWall(currentSelectedId);
                        if (newId) selectedElementId.set(newId);
                    }}
                >
                    <svg
                ...
                {#if el.type === "door" && el.door}
                    <button
                        ...
                        onclick={() => {
                            if (el.door)
                                updateDoor(el.door.id, {
                                    swingDirection:
                                        el.door.swingDirection === "left"
                                            ? "right"
                                            : "left",
                                });
                        }}
                ...
                {#if el.type === "wall" && currentSelectedId && currentSelectedIds.size === 0}
                    ...
                    >
                        <svg
                            ...
                            stroke="currentColor"
                            stroke-width="2"
                            ><path d="M12 2v20M4 12h4M16 12h4" /></svg
                        >
                    </button>
                {/if}
    ...
    {#if currentTool === "wall" && wallStart}
        <div
            class="absolute top-2 left-1/2 -translate-x-1/2 bg-blue-600 text-white px-3 py-1 rounded-full text-xs shadow"
        >
            Click to add wall segment · Double-click to finish · C to close loop
            · Esc to cancel
        </div>
    {/if}
    {#if currentPlacingId && currentTool === "furniture"}
        <div
            class="absolute top-2 left-1/2 -translate-x-1/2 bg-purple-600 text-white px-3 py-1 rounded-full text-xs shadow"
        >
            Click to place · Scroll or R to rotate ({currentPlacingRotation}°) ·
            Esc to cancel
        </div>
    {/if}
    {#if measuring}
        <div
            class="absolute top-2 left-1/2 -translate-x-1/2 bg-red-600 text-white px-3 py-1 rounded-full text-xs shadow"
        >
    ...
    <div
        class="absolute bottom-3 left-3 z-20 flex items-center gap-1 bg-white rounded-lg shadow-lg border border-gray-200 px-1 py-0.5"
    >
        <button
