<script lang="ts">
    import { undoHistoryStore, jumpToUndoStep } from "$lib/stores/layout";
...
{#if visible}
    <div
        class="fixed bottom-12 left-4 w-64 max-h-80 bg-white rounded-xl shadow-2xl border border-gray-200 z-50 flex flex-col overflow-hidden"
    >
        <!-- Header -->
        <div
                ...
                <div class="py-1">
                    {#each history.entries as entry, i}
                        <button
                            ...
                            class:text-gray-700={i < history.currentIndex &&
                                i !== history.currentIndex}
                            onclick={() => handleClick(i)}
                        >
                            <span
                                class="w-5 text-[10px] text-gray-400 text-right shrink-0"
                                >{i + 1}</span
                            >
                    ...
                    <div
                        ...
                        class:bg-blue-100={history.currentIndex ===
                            history.entries.length}
                        class:text-blue-700={history.currentIndex ===
                            history.entries.length}
                        class:text-gray-700={history.currentIndex !==
                            history.entries.length}
                    >
                        <span
                            class="w-5 text-[10px] text-gray-400 text-right shrink-0"
